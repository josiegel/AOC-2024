...
def get_nodes(coords, grid):
    ...
    for i in range(len(coords)-1):
        for j in range(i+1, len(coords)):
            ...
            if 0 <= coords[i][0] + vector[0] < len(grid) and 0 <= coords[i][1] + vector[1] < len(grid):
                nodes.add((coords[i][0] + vector[0], coords[i][1] + vector[1]))
            if 0 <= coords[j][0] - vector[0] < len(grid) and 0 <= coords[j][1] - vector[1] < len(grid):
                nodes.add((coords[j][0] - vector[0], coords[j][1] - vector[1]))
    return nodes
