...
def get_scores(start, finish, walls, direction):
    starting_node = (start, direction)
    visited_nodes = []
...
def main():
    ...
    scores = get_scores(start, finish, walls, EAST)
    flip_scores = get_scores(finish, start, walls, SOUTH)
    seats = set()
    for node in scores:
        if flip_node(node) in flip_scores and scores[node] + flip_scores[flip_node(node)] == scores[(start, EAST)] + scores[(finish, NORTH)]:
            print(node)
            seats.add(node[0])
    print(len(seats))
